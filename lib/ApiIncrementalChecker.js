...
var ApiIncrementalChecker = /** @class */ (function () {
    ...
    ApiIncrementalChecker.prototype.getEsLintIssues = function (cancellationToken) {
        return __awaiter(this, void 0, void 0, function () {
            var _i, _a, removedFile, lintableFiles, report, results, _b, results_1, _c, filePath, result, reports;
            var _this = this;
            return __generator(this, function (_d) {
                ...
                lintableFiles = this.lastUpdatedFiles.filter(function (file) { return !_this.isFileExcluded(file); });
                report = this.eslinter.getReport(lintableFiles);
                cancellationToken.throwIfCancellationRequested();
                if (report) {
                    results = report.results.map(function (reportEntry) { return [
                        reportEntry.filePath,
                        reportEntry
                    ]; });
                    for (_b = 0, results_1 = results; _b < results_1.length; _b++) {
                        _c = results_1[_b], filePath = _c[0], result = _c[1];
                        if (result.messages.length) {
                            this.currentEsLintErrors.set(filePath, __assign(__assign({}, report), { results: [result] }));
                        }
